...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($title) || empty($content) || empty($location) || empty($date) || empty($time) || empty($fb_link)) {
        exit("<span style='color: red;'>❌ All fields are required.</span>");
    }

    // Update the database
    $stmt = $conn->prepare("UPDATE news SET title = ?, content = ?, location = ?, date = ?, time = ?, fb_link = ?, image = ?, image_type = ? WHERE id = ?");
    if ($stmt) {
        $stmt->bind_param("ssssssssi", $title, $content, $location, $date, $time, $fb_link, $image, $image_type,  $id);
        if ($stmt->execute()) {
            $stmt->close();
            $conn->close();
            exit("<span style='color: green;'>✅ News updated successfully!</span>");
        } else {
            $stmt->close();
            $conn->close();
...
<div id="content" class="dashboard">
    ...
    <h3 class="dashboard-title">Edit News</h3>
    <div>
        <div class="main-cont">
                        ...
                        <div class="personal-info-header">
                            ...
                            <div class="personal-info-form">
                                ...
                                <p class="header-description wow fadeInUp" data-wow-delay="0.2s">
                                    Fill in the details below to update news information.
                                </p>
                                <div class="form-fields-container">
                                    <div class="form-fields">
                                        ...
                                        <input class="info-input wow fadeInUp" type="text" name="location"
                                            value="<?= htmlspecialchars($news['location'] ?? '') ?>"
                                        ...
                                        <input class="info-input wow fadeInUp" type="date" name="date"
                                            value="<?= htmlspecialchars($news['date'] ?? '') ?>"
                                        ...
                                        <input class="info-input wow fadeInUp" type="time" name="time"
                                            value="<?= htmlspecialchars($news['time'] ?? '') ?>"
                                            placeholder="Time of News" data-wow-delay="0.6s" />
                                        <input class="info-input wow fadeInUp" type="tel" name="fb_link"
                                            value="<?= htmlspecialchars($news['fb_link'] ?? '') ?>"
                                            placeholder="Facebook News Link" data-wow-delay="0.7s" />
                                        <input class="info-input wow fadeInUp" value="<?= htmlspecialchars($news['content'] ?? '') ?>" type="text" name="content"
                                    ...
                                    <div class="profile-image-section layout-input">
                                        <section class="upload-container">
                                            <div class="upload-wrapper">
                                                <div x-data="imageData('data:image/jpeg;base64,<?= base64_encode($news['image'] ?? '') ?>')"
